...
async function initApp() {
    try {
        ...
        // DATA QUALITY GUARDRAIL: Filter out strictly bad items only. 
        // We keep items with missing options/answers now to maintain sequential numbering for the user.
        ...
        window.allQuestions = ((typeof mcqData !== 'undefined') ? mcqData : []).filter(q => {
            ...
            return hasId && hasQuestion;
        });
        console.log(`Guardrail active: Loaded ${window.allQuestions.length}/${originalCount} questions.`);
...
function startMode(mode) {
    ...
    if (mode === 'super_revision') {
        ...
        // Sequential sorting for Study Mode (by year and question number)
        // This prevents the user from feeling that questions are "skipped"
        matching.sort((a, b) => {
        ...
        // Randomize for other modes (Custom QBank, etc.)
...
function renderQuestion() {
    ...
    if (!q) {
        console.error("No question found at index", state.currentIndex);
        return;
    }

    // Handle "Broken" or "Cancelled" questions gracefully to maintain numbering
    const isCancelled = (q.question || '').toLowerCase().includes('cancelled');
    const hasOptions = q.options && Object.keys(q.options).length >= 2; // At least some options
    
    if (isCancelled || !hasOptions) {
        // Skip header updates and images, just show placeholder
        ...
        el.quiz.qSubject.textContent = q.subject || (q.tags && q.tags[0]) || 'N/A';
        
        el.quiz.qText.innerHTML = `<div style="color: #ef4444; font-weight: bold; margin-bottom: 0.5rem;">[QUESTION REMOVED / CANCELLED]</div> ${q.question || 'This question data is unavailable.'}`;
        el.quiz.qImage.classList.add('hidden');
        el.quiz.qOptions.innerHTML = '<p style="padding: 1.5rem; background: #f9fafb; border: 1px dashed #d1d5db; border-radius: 8px; color: #6b7280; font-style: italic; text-align: center;">This question has been removed from the official paper or has insufficient data to display options. Please click "Next" to continue.</p>';
        return;
    }
...
function setupScoreListeners() {
    if (el.score.btnDashboard) {
        el.score.btnDashboard.onclick = () => {
            switchScreen('start');
        };
    }

    if (el.score.btnReview) {
        el.score.btnReview.onclick = () => {
