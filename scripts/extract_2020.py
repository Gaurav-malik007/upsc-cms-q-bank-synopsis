...
def main():
    ...
    papers = [(os.path.join(base, "new UPSC CMS 2020 paper 1.pdf"), "2020", "1"), (os.path.join(base, "new UPSC CMS 2020 paper 2.pdf"), "2020", "2")]
    all_qs = []
    for pdf, yr, num in papers:
        qs = extract_from_pdf(pdf, yr, num)
        print(f"Paper {num}: {len(qs)} questions.")
        all_qs.extend(qs)
    with open(r"c:\Users\deepa\Desktop\cms\upsc_2020_extracted.json", 'w', encoding='utf-8') as f: json.dump(all_qs, f, indent=2, ensure_ascii=False)
